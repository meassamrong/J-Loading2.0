    ...
    connect() {
      ...
      for (let i = 0; i < this.particlesArray.length; i++) {
        for (let j = i; j < this.particlesArray.length; j++) {
          const distance =
            Math.pow(this.particlesArray[i].x - this.particlesArray[j].x, 2) +
    ...
    animate() {
      ...
      for (let i = 0; i < this.particlesArray.length; i++) {
        ...
        if (dist < this.mouse.radius + particle.size) {
          ...
          if (this.mouse.x < particle.x && particle.x < this.canvas.width - buffer) {
            particle.x += 10
          }
          if (this.mouse.x > particle.x && particle.x > buffer) {
            particle.x -= 10
          }
          if (this.mouse.y < particle.y && particle.y < this.canvas.height - buffer) {
            particle.y += 10
          }
